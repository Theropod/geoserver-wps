...
@DescribeProcess(title = "nclWPS_atm", description = "Atmosphere NCL WPS Example")
public class NclWPSAtm implements GeoServerProcess {
    ...
    public String execute(@DescribeParameter(name = "ncl_function", description = "ncl function") String nclFunction,
        ...
        String workDir = currentRelativePath.toAbsolutePath() + "/data_dir/workspaces/climatemodel/nclWPS_atm/";
        // result to return
        Map<String, String> result = new HashMap<>();

        // run command, now only monsoon_index.ncl is included
        try {
            if (nclFunction.equals("monsoon_index")) {
...
//                        " obsvar=sst modelvar=tos input_model_lat=i input_model_lon=j";
                ...
                result.put("info", workDir + analyseStartYear + "-" + analyseEndYear + "-Monsoon Index.png");
